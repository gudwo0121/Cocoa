...
function selChange() {
	...
	location.href="view_projectCate?pField=${cri.pField }&level=${cri.level }&projectOrder="+sel;
}
</script>
<title>CoCoa</title>
</head>
<body style="background-color: #FFEBCD">
	...
	<section class="py-5">
		...
		<div class="container px-4 px-lg-5 mt-5">
			...
			<!-- 정렬 버튼 -->
			<select class="px-4 me-sm-3" style="float: right;" id="projectOrder" onchange="selChange()">
				<option value="projectNO desc" <c:if test="${cri.projectOrder == 'projectNO desc' }">selected</c:if>>최신순</option>
				<option value="projectNO" <c:if test="${cri.projectOrder == 'projectNO' }">selected</c:if>>오래된순</option>
				<option value="reviewRate desc" <c:if test="${cri.projectOrder == 'reviewRate' }">selected</c:if>>높은 평점순</option>
