		...
		<div class="container px-4 px-lg-5 mt-5">
						...
						<td style="vertical-align: middle;">
							...
							id="coachOrder" onchange="selChange()">
								<option value="reviewRate desc"
									<c:if test="${cri.coachOrder == 'reviewRate desc' }">selected</c:if>>평점
									높은순</option>
			...
			<div class="pb-3" style="text-align: center; font-size: 30px; width: 100%;">
				...
				<c:forEach begin="${pageMaker.startPage }"
					...
					<c:choose>
						<c:when test="${cri.page == pageNum}"><a style="text-decoration: none; color: red; font-size: 15pt;"
						href='<c:url value="/view_coachCate?cField=${cri.cField }&tool=${cri.tool }&coachOrder=${cri.coachOrder}&page=${pageNum }"/>'>${pageNum }</a>
						</c:when>
						<c:when test="${cri.page != pageNum}"><a style="text-decoration: none; color: black; font-size: 15pt;"
						href='<c:url value="/view_coachCate?cField=${cri.cField }&tool=${cri.tool }&coachOrder=${cri.coachOrder}&page=${pageNum }"/>'>${pageNum }</a>
						</c:when>
					</c:choose>
				</c:forEach>
