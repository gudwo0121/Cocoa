	...
	<div
					...
					<td colspan="4" style="align:right;">
						...
						<select class="me-sm-3"
							...
							<option value="status2"
								<c:if test="${cri.status == 'status2' }">selected</c:if>>수락한 요청</option>
							<option value="status3"
								<c:if test="${cri.status == 'status3' }">selected</c:if>>거절한 요청</option>
							<option value="status4"
								<c:if test="${cri.status == 'status4' }">selected</c:if>>코칭 완료한 요청</option>
						</select>
					</td>
					</tr>
					<tr>
		...
		<div style="margin: 0 auto; font-size: 30px;">
			...
			<c:forEach begin="${pageMaker.startPage }"
				...
				<a href='<c:url value="/view_receiveReq?status=${cri.status }&page=${pageNum }"/>'><i
					class="">${pageNum }</i></a>
			</c:forEach>
