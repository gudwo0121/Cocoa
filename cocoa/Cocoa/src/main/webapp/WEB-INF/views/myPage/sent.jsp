...
<script type="text/javascript">
	...
	function selChange() {
		...
		location.href = "view_sendReq?status="
				+ sel;
	}
</script>
	...
	<div style="text-align: center; padding: 50px; padding-left: 130px;">
		<div class="table-responsive" style="border: 1px solid grey;">
			<table class="table table-striped table-sm">
				<thead>
					<tr>
						<th colspan="4" style="text-align: center;"><b>${member.id }님이 보내신 요청 리스트</b></th>
					</tr>
					<td colspan="4" style="align:right;">
						...
						<select class="me-sm-3"
							...
							<option value="status"
								<c:if test="${cri.status == 'status' }">selected</c:if>>전체 요청글</option>
							<option value="status1"
								<c:if test="${cri.status == 'status1' }">selected</c:if>>대기중인 요청</option>
							<option value="status2"
								<c:if test="${cri.status == 'status2' }">selected</c:if>>수락된 요청</option>
							<option value="status3"
								<c:if test="${cri.status == 'status3' }">selected</c:if>>거절된 요청</option>
							<option value="status4"
								<c:if test="${cri.status == 'status4' }">selected</c:if>>코칭 완료된 요청</option>
						</select>
					</td>
					<tr>
						<th scope="col">코치</th>
		...
		<div style="margin: 0 auto; font-size: 30px;">
			...
			<c:forEach begin="${pageMaker.startPage }"
				...
				<a href='<c:url value="/view_sendReq?status=${cri.status }&page=${pageNum }"/>'><i
					class="">${pageNum }</i></a>
			</c:forEach>
