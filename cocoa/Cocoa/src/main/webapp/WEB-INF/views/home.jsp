...
<body style="background-color: #FFEBCD">
		...
		<div class="container px-4 px0lg05 my-5">
			...
			<div class="d-grid gap-3 d-sm-flex justify-content-sm-center">
				<input id="project" type="button"
					...
					onClick="location.href='/cocoa/?level=하수&level=중수&level=고수'" value="Project"> <input
	...
	<div style="margin: 0 auto; font-size: 30px;">
		...
		<c:if test="${pageMaker.prev }">
			<a
				href='<c:url value="/?level=${cri.level }&page=${pageMaker.startPage-1 }"/>'><i
				class=""></i></a>
		</c:if>

		<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }"
			var="pageNum">
			<a href='<c:url value="/?level=${cri.level }&page=${pageNum }"/>'><i
				class="">${pageNum }</i></a>
		</c:forEach>

		<c:if test="${pageMaker.next && pageMaker.endPage >0 }">
			<a
				href='<c:url value="/?level=${cri.level }&page=${pageMaker.endPage+1 }"/>'><i
				class=""></i></a>
		</c:if>
