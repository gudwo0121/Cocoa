					...
					<div class="card rcol my-5"
									...
									<c:forEach var="reviewInfo" items="${reviewList}">
										<tr style="width:100%;">
											...
											<c:if
												...
												<td style="float: right;"><input type="button" id="r_mod"
													value="수 정" onClick="fn_enable(frmReview)">&nbsp;&nbsp;<input
													type="button" id="r_rmv" value="삭 제" onClick="fn_delete(frmReview)"></td>
											</c:if>
										</tr>
										<tr>
										...
										<tr>
											<!-- 후기 한 줄 사이의 간격 -->
											<td colspan="5" style="float:right;"><input type="button" id="r_modBtn"
													value="확 인" onClick="fn_update(frmReview)"></td>
										</tr>
									</c:forEach>
								</table>
							</form>
						...
						<div style="margin: 0 auto; font-size: 30px;">
							...
							<c:forEach begin="${pageMaker.startPage }"
								...
								<a href='<c:url value="/view_reviewInfo?target=${target}&page=${pageNum }"/>'><i
									class="">${pageNum }</i></a>
							</c:forEach>
